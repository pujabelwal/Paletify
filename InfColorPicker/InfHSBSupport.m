...
void HSVtoRGBn( float h, float s, float v, float* r, float* g, float* b )
{
...
void RGBToHSVn( float r, float g, float b, float* h, float* s, float* v, BOOL preserveHS )
{		
	float max = r;
	if( max < g )
		max = g;
...
CGImageRef createHSVBarContentImage( InfComponentIndex barComponentIndex, float hsv[ 3 ] )
{
	...
	for( int x = 0 ; x < 256 ; ++x ) {
		...
		HSVtoRGBn( hsv[ 0 ] * 360.0f, hsv[ 1 ], hsv[ 2 ], &r, &g, &b );
